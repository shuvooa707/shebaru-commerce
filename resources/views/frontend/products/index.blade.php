                        ...
                        <div class="axil-shop-sidebar">
                            ...
                            <div class="toggle-list product-categories active">
                                ...
                                <div class="shop-submenu">
                                    <ul>
                                        @foreach($cats as $cat)
                                            <li class="category {{ request('category_id') == $cat->id?'current-cat':''}}"
                                                data-value="{{ $cat->id}}"><a> {{ $cat->name}} </a></li>
                                        @endforeach
                                    </ul>
                                </div>
                            </div>
                            ...
                            <div class="toggle-list product-vendors active">
                                <h6 class="title">VENDORS</h6>
                                <div class="shop-submenu">
                                    <ul>
                                        @foreach($vendors as $vendor)
                                            <li class="vendor {{ request('vendor_id') == $vendor->id ? 'current-cat' : ''}}"
                                                data-value="{{ $vendor->id }}"><a> {{ $vendor->name}} </a></li>
                                        @endforeach
                                    </ul>
                                </div>
                            </div>
                                ...
                                <div class="shop-submenu">
                                    <ul>
                                        @foreach($types as $type)
                                            <li class="brand {{ request('brand_id') == $type->id?'current-cat':''}}"
                                                data-value="{{ $type->id}}"><a> {{ $type->name}} </a></li>
                                        @endforeach
                                    </ul>
                                </div>
                            </div>
...
@push('js')
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            $('#amount').val('ট' + $('#slider-range').slider('values', 0) + '  ট' + $('#slider-range').slider('values', 1));
        })
        ...
        function fetchData(page = null) {
            ...
            var vendor = $('li.vendor.current-cat').map(function () {
                return $(this).data('value');
            });
            var vendor_id = vendor.get();
            ...
            $.ajax({
                ...
                data: {cat_id, size_id, q, shorting, brand_id, vendor_id},
